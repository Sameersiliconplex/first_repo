...
if (isset($_GET['SubmitAdd'])) {
    ...
    //////////Putting values in Variable & Session//////////////////////////
    ...
    $appartment = $_GET['ApartmentNo'];
    $floor = $_GET['FloorLevel'];
    $street = $_GET['StreetNo'];
    $street_level = $_GET['StreetLetter'];

    $rent = $_GET['RentAmount'];
    $bed = $_GET['Bedrooms'];
    $deposite = $_GET['DepositAmount'];
    $bath = $_GET['Bathrooms'];

    $utility = $_GET['Utilities'];
    $building = $_GET['BuildingTypeID'];
    //skip dates
    $room_furnished = $_GET['IsRoomFurnished'];
    $date_negotiable = $_GET['DateNegotiable'];

    //leasetype
    $leasetype = $_GET['LeaseType'];
    $lease_type_value = $_GET['LeaseType_Value'];

    // sq.fts and intersection
    $fts = $_GET['sqft'];
    $inter = $_GET['intersection'];

    //last four field
    $additional_details = $_GET['additionalDetails']; //*
    $im_looking_for = $_GET['LookingFor']; //*
    $maximum_number = $_GET['NumberOfMembers']; //*
    $preference = $_GET['Preference'];

    //date and time
    $date = $_GET['AvailDay'];
    $month = $_GET['AvailMonth'];
    $year = $_GET['AvailYear'];

    //  1)required checkbox
    
    $required = json_encode($_GET['Required']);
    $required_serialize=$required;
    $security_amount = $_GET['SecurityAmount'];
    $brokerfee_amount = $_GET['BrokerFeeAmount'];

    ///  2)HRES Apartment Source Pays For
    $no_utilities = $_GET['No_Utilities'];
    $hres_apartment = json_encode($_GET['hres_apartment']);
    // var_dump($hres_apartment);
    // die;

    //  3)rental_features
    $rental_features = json_encode($_GET['rental_features2']);

    //  4)community_features2
    $other_features = json_encode($_GET['other_features']);

    //  5)Parking_Section
    $parking_section = json_encode($_GET['Parking_Section2']);
   
    $garage_amount = $_GET['GarageAmount'];
    $off_street_amount = $_GET['OffstreetAmount'];

    //Date and time
    $availmonth = $_GET['dateandtime'];

    // if ($property_name_error && $city_error && $zip_error &&  $state_error &&
    // $rent_error &&  $im_looking_for_error &&  $fts_error &&
    // $additional_details_error &&  $maximum_number_error &&  $bath_error &&  $inter_error
    // ) {
    ...
    $property_name_error = empty($_GET['propertyName']) ? "This Field is Required" : "";
    ...
    if (empty($property_name_error) && empty($city_error) && empty($zip_error) && empty($state_error) &&
      ...
      header("Location:view.php?property_name=$property_name&location=$location&city=$city&state=$state&zip=$zip&appartment=$appartment&floor=$floor&street=$street&street_level=$street_level&rent=$rent&bed=$bed&deposite=$deposite&bath=$bath&utility=$utility&building=$building&room_furnished=$room_furnished&date_negotiable=$date_negotiable&leasetype=$leasetype&lease_type_value=$lease_type_value&fts=$fts&inter=$inter&additional_details=$additional_details&im_looking_for=$im_looking_for&maximum_number=$maximum_number&preference=$preference&dated=$date&month=$month&year=$year&required=$required_serialize&security_amount=$security_amount&brokerfee_amount=$brokerfee_amount&$no_utilities=No_Utilities&hres_apartment=$hres_apartment&rental_features=$rental_features&other_features=$other_features&Parking_Section2=$parking_section&GarageAmount=$garage_amount&OffstreetAmount=$off_street_amount");
        ...
        window.open("ZipCodeLocator.asp?Form=NewProfile&Field=ZipCode","zipcode",'toolbar=no,status=no,scrollbars=no,resizable=no,location=no,menubar=no,directories=no,width=200,height=250');
            ...
            <TABLE cellSpacing=0 width=615>
              ...
              <TR>
                ...
                width=7></TD>
                <TD>
                  <FORM  action="" name=NewProfile method=get>
